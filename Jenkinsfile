pipeline {
    agent any
    environment {
        DOCKER_HUB_REPO = "hemanthng/studybuddy_ai"
    ...
    stages {
        stage('Checkout Github') {
            steps {
                ...
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'github-token', url: 'https://github.com/HemanthNeehar/Study_Buddy_Planner.git']])}
        }        
        ...
        stage('Update Deployment YAML with New Tag') {
            steps {
                script {
                    sh """
                    sed -i 's|image: hemanthng/studybuddy_ai:.*|image: hemanthng/studybuddy_ai:${IMAGE_TAG}|' manifests/deployment.yaml
                    """
                }
            }
        }
